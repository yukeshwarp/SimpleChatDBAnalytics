...
with tab1:
        ...
        if filter_option == "Date Range":
            ...
            res = list(
                container.query_items(
                    query=query_len, enable_cross_partition_query=True
                )
            )
            num_ent = res[0]
            limit = st.slider(
        ...
        if fetch_button:
            try:
                if filter_option == "Date Range":
                    ...
                    start_date_str = start_date_obj.strftime(
    ...
    chat_titles_text = "\n".join(
        chat_titles
    )  # Join chat titles into a single text block
    ...
    if chat_titles:
        ...
        st.session_state["messages"].append(
            {"role": "assistant", "content": trend_analysis}
        )
...
# Analytics Tab
with tab2:
    # st.title("Quarterly Topic Stats")
    ...
    def get_top_topics(start_date, end_date):
        ...
        chat_titles = "\n".join([item["ChatTitle"][:100] for item in items])
